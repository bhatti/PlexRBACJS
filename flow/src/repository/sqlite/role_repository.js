    ...
    async save(role: IRole): Promise<IRole> {
        ...
        let savePromise = new Promise((resolve, reject) => {
            if (role.id) {
                ...
                let stmt = this.dbFactory.db.prepare('INSERT INTO roles (realm_id, role_name) VALUES (?, ?)');
        ...
        role.parents.forEach(parent => {
            if (parent.id != role.id) {
                insertPromises.push(new Promise((resolve, reject) => {
                    this.dbFactory.db.run('INSERT INTO role_parents (role_id, parent_role_id) VALUES(?, ?) ',
    ...
    async __savePrincipalRoles(principal: IPrincipal): Promise<IPrincipal> {
        ...
        principal.roles.forEach(role => {
            ...
            savePromises.push(new Promise((resolve, reject) => {
              this.dbFactory.db.run('INSERT INTO principals_roles (principal_id, role_id, start_date, end_date) VALUES (?, ?, ?, ?)',
