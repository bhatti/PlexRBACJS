...
export class PrincipalRepositorySqlite implements PrincipalRepository {
    ...
    constructor(theDBFactory: DBFactory,
        ...
        theLimitRepository: LimitRepository,
        ...
        assert(theLimitRepository, 'limit-repository not specified');
        ...
        this.limitRepository = theLimitRepository;
        this.cache              = theCache;
        this.sqlPrefix          = 'SELECT rowid AS id, principal_name, realm_id, properties FROM principals';
    }
    ...
    async save(principal: IPrincipal): Promise<IPrincipal> {
        ...
        let savePrincipal = new Promise((resolve, reject) => {
            if (principal.id) {
                if (loaded && loaded.principalName !== principal.principalName) {
                    let stmt = this.dbFactory.db.prepare('UPDATE principals SET principal_name = ?, properties = ? WHERE rowid = ?');
                    stmt.run(principal.principalName, JSON.stringify(principal.properties), principal.realm().id);
                    stmt.finalize(err => {
                        if (err) {
                ...
                let stmt = this.dbFactory.db.prepare('INSERT INTO principals (realm_id, principal_name, properties) VALUES (?, ?, ?)');
                stmt.run(principal.realm().id, principal.principalName, JSON.stringify(principal.properties), function(err) {
        ...
        await this.limitRepository.__savePrincipalLimits(principal);
    ...
    async removeById(id: number): Promise<boolean> {
        ...
        let claimPromise = new Promise((resolve, reject) => {
                            this.dbFactory.db.run('DELETE FROM principals_claims WHERE principal_id = ?', id, (err) => {
                                  if (err) {
                                        reject(new PersistenceError(`Failed to remove principal-claims ${id}`));
                                  } else {
                                        resolve(true);
                                  }
                            });
        ...
        let rolePromise = new Promise((resolve, reject) => {
                            this.dbFactory.db.run('DELETE FROM principals_roles WHERE principal_id = ?', id, (err) => {
                                  if (err) {
                                        reject(new PersistenceError(`Failed to remove principal-roles ${id}`));
                                  } else {
                                        resolve(true);
                                  }
                            });
                  });
                  let limitPromise = new Promise((resolve, reject) => {
                                                this.dbFactory.db.run('DELETE FROM limits WHERE principal_id = ?', id, (err) => {
                                                      if (err) {
                                                            reject(new PersistenceError(`Failed to remove Principal-limits ${id}`));
                                                      } else {
                                                            resolve(true);
                                                      }
                                                });
                                      });
        await Promise.all([mainPromise, claimPromise, rolePromise, limitPromise]);
        return removed;
    }
